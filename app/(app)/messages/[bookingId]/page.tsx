...
export default async function CustomerThreadPage({ params }: { params: Promise<{ bookingId: string }> }) {
  const { bookingId } = await params;
  const user = await requireRole("CUSTOMER");
  const [thread, booking] = await Promise.all([
    getThread(bookingId, user.id, user.role),
    getBookingForUser(bookingId, user.id, "CUSTOMER"),
  ]);

  return (
    <>
      <BackButton fallback="/messages" label="All messages" />

      <div className="page-head row-between" style={{ marginTop: 16 }}>
        <div className="row" style={{ gap: 12 }}>
          <Avatar name={thread.counterpart} src={booking.provider.user.avatar} size="lg" />
          <div>
            <h1>{thread.counterpart}</h1>
            <p className="muted small">
              {thread.serviceName} · {formatDate(booking.scheduledDate)} at {formatTime(booking.startTime)}
            </p>
          </div>
        </div>
        <div className="row" style={{ gap: 8 }}>
          <StatusBadge status={booking.status} label={BOOKING_LABEL[booking.status]} />
          <Link href={`/requests/${booking.requestId}`} className="btn btn-secondary btn-sm">Job details</Link>
        </div>
      </div>
